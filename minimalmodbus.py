...
class Instrument:
    ...
    def _genericCommand(
        ...
        ALL_PAYLOADFORMATS = [
            PAYLOADFORMAT_LONG,
            PAYLOADFORMAT_FLOAT,
            PAYLOADFORMAT_STRING,
            PAYLOADFORMAT_REGISTER,
        ...
        if (
            functioncode == 16
            and payloadformat == PAYLOADFORMAT_REGISTER
        ...
        if functioncode in [5, 6, 15, 16]:
            _checkResponseRegisterAddress(
                payloadFromSlave, registeraddress
            )  # response register address

        if functioncode == 5:
            _checkResponseWriteData(
                payloadFromSlave, _createBitpattern(functioncode, value)
            )  # response write data
        ...
        if functioncode == 15:
            _checkResponseNumberOfRegisters(
                payloadFromSlave, NUMBER_OF_REQUESTED_BITS
    ...
    def _performCommand(self, functioncode, payloadToSlave):
        ...
        # Extract payload
        payloadFromSlave = _extractPayload(
...
def _embedPayload(slaveaddress, mode, functioncode, payloaddata):
    ...
    firstPart = (
        _numToOneByteString(slaveaddress)
        + _numToOneByteString(functioncode)
        + payloaddata
    )
...
def _calculate_minimum_silent_period(baudrate):
    ...
    """
    _checkNumerical(
        baudrate, minvalue=1, description="baudrate"
...
def _hexdecode(hexstring):
    ...
    if sys.version_info[0] > 2:
        ...
        try:
            ...
            raise TypeError(
                "Hexdecode reported an error: {}. Input hexstring: {}".format(
                    err.message, hexstring
                )
            )
...
def _checkResponseNumberOfRegisters(payload, numberOfRegisters):
    ...
    _checkInt(
        numberOfRegisters, minvalue=1, maxvalue=0xFFFF, description="numberOfRegisters"
    )
